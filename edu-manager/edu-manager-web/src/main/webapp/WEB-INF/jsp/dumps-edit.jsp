...
<div class="super-theme-example">
	<form id="ffDumpsEditForm" method="post">
...
<script type="text/javascript">
	var ffDumpsEditFormEditDesc;
	var ffDumpsEditFormEditSuperiority;
	var ffDumpsEditFormEditFaqs;
	$(function() {
		ffDumpsEditFormEditDesc = ZCN
				.createEditor("#ffDumpsEditForm [name=dumpdesc]");
		ffDumpsEditFormEditSuperiority = ZCN
				.createEditor("#ffDumpsEditForm [name=superiority]");
		ffDumpsEditFormEditFaqs = ZCN
				.createEditor("#ffDumpsEditForm [name=faqs]");
	});

	//提交表单
	function submitForm() {
		//有效性验证
		if (!$('#ffDumpsEditForm').form('validate')) {
			$.messager.alert('提示', '表单还未填写完成!');
			return;
		}
		//取价格，单位为“分”
		//$("#ffDumpsEditForm [name=price]").val(eval($("#ffDumpsEditForm [name=priceView]").val()) * 100);
		//同步文本框中的商品描述
		ffDumpsEditFormEditDesc.sync();
		ffDumpsEditFormEditSuperiority.sync();
		ffDumpsEditFormEditFaqs.sync();
		//ajax的post方式提交表单
		//$("#itemAddForm").serialize()将表单序列号为key-value形式的字符串
		alert($("#ffDumpsEditForm").serialize());
		$.post("/dumps/save", $("#ffDumpsEditForm").serialize(),
				function(data) {
					if (data.status == 200) {
						$("#dgDumps").datagrid("reload");
						$.messager.alert('操作成功', '恭喜您修改产品成功', 'warning',
								function() {
									$("#dumpsEditWindow").window('close');
									$('.dumps-list').click();
								});
					}
				});
