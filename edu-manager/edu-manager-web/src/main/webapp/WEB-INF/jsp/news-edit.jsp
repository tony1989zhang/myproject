...
<div class="super-theme-example">
	<form id="ffNewsEditForm" method="post">
		<input type="hidden" name="id" />
		<div class="form-item">
			<label for="" class="label-top">标题：</label> <input
...
<script type="text/javascript">
	var ffNewsEditFormEditDesc;
	$(function() {
		ffNewsEditFormEditDesc = ZCN
				.createEditor("#ffNewsEditForm [name=newscontent]");
	...
	function submitForm() {
		//有效性验证
		if (!$('#ffNewsEditForm').form('validate')) {
			$.messager.alert('提示', '表单还未填写完成!');
			return;
		}
		//取价格，单位为“分”
		//$("#ffNewsEditForm [name=price]").val(eval($("#ffNewsEditForm [name=priceView]").val()) * 100);
		//同步文本框中的商品描述
		ffNewsEditFormEditDesc.sync();
		
		//ajax的post方式提交表单
		//$("#itemAddForm").serialize()将表单序列号为key-value形式的字符串
// 		alert($("#ffNewsEditForm").serialize());
		$.post("/news/save", $("#ffNewsEditForm").serialize(),
				function(data) {
					if (data.status == 200) {
						$("#dgNews").datagrid("reload");
						$.messager.alert('操作成功', '恭喜您修改产品成功', 'warning',
								function() {
									$("#newsEditWindow").window('close');
									$('.news-list').click();
								});
					}
				});
