...
<div class="super-theme-example">
	<form id="ffBannerAdd" method="post">
		<div class="form-item">
			...
			<input class="easyui-textbox" type="text" name="bannername"
		...
		<div class="form-item">
			<label for="" class="label-top">广告添加：</label>
			...
			<input type="hidden" name="bannerurl" /> <a
...
<script>
	...
	function submitForm() {
		...
		//有效性验证
		var isValidate = $('#ffBannerAdd').form('validate');
		if (!isValidate) {
			$.messager.alert('警告', '没有填写完全', 'warning');
			return;
		}

		$.post("/banner/save", $("#ffBannerAdd").serialize(), function(data) {
			if (data.status == 200) {
				$("#dg").datagrid("reload");
				$.messager.alert('操作成功', '恭喜您添加一条数据成功', 'warning', function() {
					$('.banner-list').click();
				});
			}
		});
